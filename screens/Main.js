...
//Add data object variable(in Main.js):
const data =[
  ...
  {id:"flower-pic2",src:require("../assets/crown-pic2.png") },
]
...
export default class Main extends Component {
	...
	render() {
		...
		return (
          ...
          else if(this.state.current_filter ==="crown-pic4"){
            return <Filter4 key={face.faceID} face={face}/>
          }
          else if(this.state.current_filter ==="crown-pic5"){
					...
					<View style={styles.lowerBottomContainer}>
						<ScrollView
							...
							{data.map((filter_data) => {
								return (
					<TouchableOpacity
          key={`filter-button-4{filter_data.id}`}
          style={[
            styles.filterButton,
            {
              borderColor:
